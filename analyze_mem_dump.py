...
def GearChunking(dumpfile, magicval):
  ...
  mask = (2 ** (num_significant_bits)-1) << (32 - num_significant_bits)#GetMask(num_significant_bits)
  print(mask)
  ...
  while pos < l:
    ...
    if finger_print & mask == magicval:
      ...
      '''new_byte = b""
      for x in range(last+1,numbytes):
        new_byte = new_byte + byte_array[x]
      new_byte = new_byte.decode("utf-8")'''
...
if __name__ == "__main__":
  ...
  strg = GearChunking("new_dump.dump",mask)
